...
function createVideoCarousel(videos, targetId, options = {}) {
    ...
    const { controls = true, interval = 10000, ratioTemplate = "a4x3demo", aspectRatio = null } = options;
    ...
    videos.forEach((video, index) => {
        ...
        videoFrame.className = `video-aspect-frame ${aspectRatio ? '' : ratioTemplate}`.trim();
        if (aspectRatio) {
            videoFrame.style.aspectRatio = aspectRatio;
        }

        const videoElement = document.createElement('video');
        videoElement.className = 'video-carousel-video';
        ...
        innerCol.appendChild(videoFrame);
